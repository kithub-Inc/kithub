...
const CreateRepository = (): JSX.Element => {
    ...
    const handleSubmit = async (): Promise<void> => {
        ...
        if (name.trim() !== `` && name.length <= 20 && description.length <= 100 && category.trim() !== ``) {
            ...
            const response = await axios.post(`${process.env.BACKEND_URL}/api/repository/create`, formdata, { headers: { 'Content-Type': `multipart/form-data` } });
        ...
        <main className="grid flex-1 items-start gap-4 p-4 sm:px-6 sm:py-0 md:gap-8 mt-10">
            <div className="mx-auto grid max-w-[59rem] flex-1 auto-rows-max gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className="flex-1 shrink-0 whitespace-nowrap text-xl font-semibold tracking-tight sm:grow-0">레포지토리 생성</h1>
                ...
                <div className="flex items-center justify-center gap-2 md:hidden">
                    ...
                    <Button size="sm" onClick={handleSubmit}>레포지토리 생성</Button>
                </div>
            </div>
        </main>
    );

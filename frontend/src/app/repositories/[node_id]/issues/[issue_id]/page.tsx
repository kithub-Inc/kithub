...
import { AntennaIcon, BugIcon, CornerDownRightIcon, HeartIcon, XIcon } from 'lucide-react';
...
const RepositoryIssue = (props: any): JSX.Element => {
    ...
    useEffect(() => {
        (async () => {
            const responseIssue = await axios.get(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${props.params.issue_id}`);
            if (responseIssue.data.status === 200) {
                const responseComments = await axios.get(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${responseIssue.data.data.node_id}/comments`);
                if (responseComments.data.status === 200) setComments(responseComments.data.data);

                setIssue(responseIssue.data.data);
            }
        })();
    }, [props.params.issue_id, props.params.node_id]);

    const handleCommentSubmit = async (): Promise<void> => {
        if (comment.current) {
            const response = await axios.post(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${issue?.node_id}/comment/create`, { accessToken: localStorage.getItem(`accessToken`), comment_content: comment.current.value, reply: reply?.node_id });
    ...
    const handleHeartsSubmit = (value: number) => {
        return () => {
            axios.post(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${issue?.node_id}/comment/${value}/heart`, { accessToken: localStorage.getItem(`accessToken`) }).then(e => {
    ...
    const handleStatusChange = async (value: string): Promise<void> => {
        const response = await axios.post(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${props.params.issue_id}/status`, { accessToken: localStorage.getItem(`accessToken`), status: value });
        if (response.data.status === 200) {
            const responseIssue = await axios.get(`${process.env.BACKEND_URL}/api/repository/${props.params.node_id}/issue/${props.params.issue_id}`);
            if (responseIssue.data.status === 200) setIssue(responseIssue.data.data);
        }
    }
